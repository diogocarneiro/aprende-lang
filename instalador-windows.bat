...
echo            📧 Email:    diogocarneiro@gmail.com
...
code --list-extensions | findstr "diogocarneiro.aprende-lang" >nul 2>&1
if errorlevel 1 (
    echo ❌ Extensão Aprende não instalada
) else (
    echo ✅ Extensão Aprende instalada
)
...
echo 🎨 Instalando extensão VS Code...
if not exist "vscode-extension\aprende-lang-1.1.1.vsix" (
    echo ❌ Ficheiro da extensão não encontrado!
    echo 📁 Certifique-se que está na pasta raiz do projeto Aprende
    exit /b 1
)

code --install-extension "vscode-extension\aprende-lang-1.1.1.vsix" --force >nul 2>&1
if errorlevel 1 (
...
if not exist "examples\teste-estrutura.aprende" (
    echo ❌ Ficheiros de teste não encontrados!
    exit /b 1
)
